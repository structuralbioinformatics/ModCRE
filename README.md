...
#[warning] Modify the scripts to load the program versions that you have installed, according to the definotions use in the configuration file
sh pdb_1.sh
sh pdb_2.sh
sh pdb_3.sh
sh pdb_4.sh
...
-----------------------
# [warning] if the tfs do not dimerize, it won't model anything (see next example)
...
python scripts/xprofiler.py --chains_fixed --dummy=./dummy -d example/protein2dna/ERR1P/dna_large.fa -i example/protein2dna/TF_profiler_ESR1_ERR1.dat  --pbm=./pbm_2.0 --pdb=./pdb \
 -v  --plot --html --html_types normal,energy,energy_per_nucleotide,energy_best,fimo_log_score,fimo_binding,fimo_score --html_energies s3dc_dd,s3dc,pair   -o  ESR1_ERR1.profile \
 -l thread --auto  --parallel --radius 22.0 --reuse
